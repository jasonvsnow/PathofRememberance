...
public class Game {
	...
	public void drinkHPotion() {
		if (hero.getHP() > 0) {
			if (hero.getHPPotion() > 0) {
				...
				else Help.print("You're already at maximum health! Save that potion for later.\n\n");
			}
			else Help.print("You don't have any health potions to drink.\n\n");
		}
	}
	public void drinkMNPotion() {
		if (hero.getHP() > 0) {
			if (hero.getMNPotion() > 0) {
				...
				else Help.print("You're already at maximum mana! Save that potion for later.\n\n");
			}
			else Help.print("You don't have any mana potions to drink.\n\n");
		}
	}
	...
	public int room(int room, int choice) {
		...
		//Awakening Room
			...
			if (choice == 1) {
				if (hero.isMap()) {
					if (hero.isTutorial()) {
						Help.print("You insert the key into the door and twist, unlocking the door and swinging it open with a creak to enter the armory.\n");
						...
						Help.print("You leave through the door to enter the armory once agian.");
						hero.setRoom(2);
						enterRoom();
					}
		...
		//armory
			...
			if (choice == 1) {
				Help.print("You exit the armory via a sturdy, reinforced wooden door, entering the barracks.\n");
				hero.setRoom(3);
				combatStarter(1);
				type = 1;
			}
			else if (choice == 2) {
				Help.print("You exit the armory through a heavy metal door, entering the awakening room once more.\n");
				hero.setRoom(1);
				enterRoom();
			}
			else if (choice == 3) {
		...
		//barracks
			...
			if (choice == 1) {
				Help.print("You approach the northern wall which has no visible door as the map might indicate, at least until you step near it- "
				...
				Help.print("The door to the storage is partially broken, the latch failed so that it is left to hang open, needing only a gentle pull to allow entrance and as you step inside it swings closed behind you.\n");
				hero.setRoom(5);
				...
				Help.print("The hallway entrance is a very plain and simple door, with no ability to lock or bar it naturally a part of its function. It creaks softly as you open it to step into the hallway.\n");
				hero.setRoom(6);
				type = (int)(Math.random()*3);
				if (type > 0) combatStarter(type);
				else enterRoom();
				...
				Help.print("You return to the armory via the heavy sturdy, reinforced door.\n");
				hero.setRoom(2);
				enterRoom();
			}
		}
		//shop
			...
			if (choice == 1) {
				if (hero.getCoins() > 0) {
					Help.print("You give the man a gold coin and in exchange receive a single bottle full of red liquid. It probably tastes like strawberries. Hopefully.\n"
							+ "1) Buy a potion of health(1 coin)\n"
							+ "2) Buy a potion of mana.(2 coins)\n");
					if (!hero.isCaptainKey()) {
						Help.print("3) Buy the golden key.(10 coins)\n"
							+ "4) Leave\n"
							+ "\n");
					}
					else Help.print("3) Leave\n"
							+ "\n");
					hero.setHPPotion(hero.getHPPotion()+1);
					hero.setCoins(hero.getCoins()-1);
				}
				else Help.print("You go to grab a coin to offer the man, but find you have none. You'll have to come back later.\n"
						+ "1) Buy a potion of health(1 coin)\n"
						+ "2) Buy a potion of mana.(2 coins)\n");
				if (!hero.isCaptainKey()) {
					Help.print("3) Buy the golden key.(10 coins)\n"
				...
				if (hero.getCoins() > 1) {
					Help.print("You give the man a gold coin and in exchange receive a single bottle full of shimmering light. Probably feels weird to drink.\n"
							+ "1) Buy a potion of health(1 coin)\n"
							+ "2) Buy a potion of mana.(2 coins)\n");
					if (!hero.isCaptainKey()) {
						Help.print("3) Buy the golden key.(10 coins)\n"
		...
		//storage
			...
			if (choice == 1) {
				Help.print("You return to the barracks, pushing aside the busted door.\n");
				hero.setRoom(3);
				type = (int)(Math.random()*3);
				if (type > 0) combatStarter(type);
				else enterRoom();
				...
				Help.print("You find nothing, for now.\n");
			}
		}
		//hallway
		else if (room == 6) {
		...
		//mess hall
			...
			if (choice == 1) {
				Help.print("You will not give into the darkness! Though it surrounds you now, you realize you are but sleeping.\n\n\n"
						+ "\n");
				hero = new Character();
				reRoom = 0;
	...
	public void combatStarter(int seed) {
		reRoom = 8;
		if (seed == 0) {
			Help.print("As you step into the armory, a shriveled little green creature turns to you and howls, lunging at you with murder in its beady red eyes.\n");
		}
		else if (seed == 1) {
			Help.print("A deep groan echoes near you. Turning, you see what looks to be a person wearing some uniform and carrying an old sword."
			...
			Help.print("There is a sound like metal scrapping as a nearby spike splits open and out crawls a creature. "
	...
	public void enterRoom() {
		...
		if (reRoom == 1) {
			//Awakening room
			Help.print("\tThe awakening room is as you left it: the obsidian like table in the center, strange runes scrawled upon the floor, now empty desk tucked into the corner.\n"
					+ "1) Go to the armory\n"
					+ "\n");
		}
		else if (reRoom == 2) {
			//armory 
			Help.print("\tThe armory's floor and walls are made of many heavy stone blocks cemented together. "
					...
					+ "1) Go to the barracks\n"
					+ "2) Go to the awakening room.\n");
					if (hero.getAttack() <= 2) Help.print("3) Search for better equipment.\n");
					Help.print("\n");	
		}
		else if (reRoom == 3) {
			//barracks
			Help.print("\tThe barracks walls are the same heavy stone, but its floor is wooden boards- boards pierced, shattered, and pushed aside by spikes of various sizes. "
			...
			//shop
			Help.print("\tAs you step through the entrance made in the wall, you find yourself in a small room that actually holds its own lightning provided by the four braziers that sit in the corners of the room which has wooden log walls, smooth boards upon the floor, and rafters instead of a proper ceiling. "
			...
			//storage
			Help.print("\tThe storage room has a single, massive black spike jutting through it up from the floor into the ceiling above. "
			...
			//hallway
			Help.print("\tThe hallway’s stone floor is broken and shattered by the many spikes jutting throughout it, out of and into walls and floors alike, making navigating it a little difficult.\n"
					+ "1) Go to the barracks\n"
			...
			//training room
			Help.print("\tThe training room holds a pit of sand with an elevated wooden walkway around it. "
			...
			Help.print("\tAs you enter the captian's chamber, you realize you won.");
			
		}
		else if (reRoom == 10) {
			//mess hall
			Help.print("\tThe mess hall’s floors shift from the stone of the hallway back to the same wood of the barracks. "
