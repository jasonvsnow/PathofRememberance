...
public class PathofRememberance {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Printer.println("You come to consciousness and feel yourself lying upon something hard. You are surrounded by darkness, unable to see anything.");
		Printer.println("");
		boolean dark = true;
		while (dark == true) {
		...
		while (appearance) {
			...
			boolean clothe = true;
			while (clothe) {
				Printer.println("You are wearing: ");
				...
				if (choice == 1) {
					...
					while (invest == true) {
						...
						if (choice == 1) {
							...
							Printer.println("\"It presses upon us presently, as constant as ever. We shall not escape this, but all hope is not lost. We place our faith in our greatest warrior to deliver us from this.\"");
							Printer.println("The rest of the page has been smeared and torn, rendering it illegible.");
						}
						else if (choice == 2) {
							invest = false;
						}
		...
		if (hero.getHP() > 0) Printer.println("With the wisp dealt with, you are able to proceed forward and find yourself in another room."); 
		Printer.println("");
		hero.setRoom(2);
		int room = 2;
	...
	/**
	 * This method represented the area a player could explore and would take the player object and a room number. It would run that player through that room and return a number based on what the player chose. That number would lead to a new room.
	 * <pre>Example:
	...
	public static int rooms(int room, Player hero) throws InterruptedException {
		...
		if (room == 1) {
			...
			while (room1 == true && hero.getHP() > 0) {
				...
				Printer.println("5) Check resources");
				...
				if (choice == 1) {
					...
					if (hero.getPotions() > 0) {
						...
						else Printer.println("You are already at maximum health, better to save that potion for later.");
					}
					else Printer.println("Invalid Input. Try again.");
				}
				else if (choice == 3) {
		...
		//armory
			...
			while (room2 == true && hero.getHP() > 0) {
				...
				Printer.println("6) Check resources");
					...
					if (test == 5) {
						...
						while (weapons) {
							...
							if (choice == 1) {
								...
								Printer.println("You take the battleaxe, lacking in defense but great for destruction.");
								hero.setShield(0);
								hero.setDMG(12);
								if (hero.getMana() > 10) hero.setMana(10);
								weapons = false;
								...
								Printer.println("You take the staff, subpar for fighting but great for magic.");
								hero.setShield(0);
								hero.setDMG(4);
		...
		//eastern barracks
			...
			Printer.println("You enter the East Barracks. There are four doors leading out of this room, one in each direction. The one to the north doesn't seem to stick in your memory as the others here have, as if it does not belong. Bunks fill the room in rows, two sets of bunks two high with a dresser between them, each dresser holding four drawers. The beds are in disarray, there are clothes hanging out of unclosed drawers. There are even some stray items upon beds, left out as if people had simply forgotten them.");
			Enemy awaken = new Enemy();
			...
			while (room3 == true && hero.getHP() > 0) {
				...
				Printer.println("7) Check resources");
				if (hero.getPotions() > 0) Printer.println("8) Drink a potion");
				Printer.print("> ");
				try {
					choice = input.nextInt();
					Printer.println("");
				...
				if (choice == 1) {
					Printer.println("You rummage through the drawers and beds. You find a few useless items- playing cards, pictures of people you do not recognize, a pair of glasses- but you also find a diary. The ink is smeared but you find yourself able to read some of the pages.");
			...
			while (room4 == true && hero.getHP() > 0) {
				...
				if (choice == 1) {
					...
					Printer.println("\"The enemy has arrived in great number, men and women alike, even their wretched offspring. I grow more uneasy with each passing moment. Something is bound to happen, it is only a matter of time.\"");
				}
				else if (choice == 3) {
		...
		//room 5 east storage
			...
			Printer.println("Stepping into the East Storage room, you find a great disarray. Everything once stored here is flung about, the palace ransacked. Racks thrown down, chests burst open, contents strewn about. Cloaks and uniforms are tossed about. There are a number of flasks spilled, their contents long ago staining the ground, there are boots, weapons, broken weapons, personal effects, books, and even some rope. ");
			Enemy awaken = new Enemy();
			int pop = 1;
			createEnemy(awaken, pop);
			combat(hero, awaken);
			...
			while (room5 == true && hero.getHP() > 0) {
				...
				Printer.println("4) Check resources");
		...
		//room 6 west storage
			...
			Printer.println("Stepping into the West Storage room, you find many crates and barrels, clearly a storage for more extended periods of time. One barrel has its bottom shattered, potatoes spilling out over the floor. A trail of these potatoes leads further into the room, between two large crates.");
			Enemy awaken = new Enemy();
			...
			while (room6 == true && hero.getHP() > 0) {
				...
				if (choice == 1) {
					...
					if (hero.getSquid() == 0) {
						...
						Printer.println("There is a yelp from the creature you have surprised. You see something small, no taller than 3 feet, with pink skin, tattered clothes, and a face that has no nose or mouth, instead tentacles hang off the thing's face from beneath its massive, pure black eyes. It holds a potato to its chest with a three fingered, clawed hand, shivering.");
					}
						...
						Printer.println("You follow the trail of potatoes to find the little squid creature sitting there, munching upon a potato with your cloak upon it. It gurgles happily upon seeing you and offers you a potato. You take some time to sit and enjoy a potato with the little thing. After you finish your small meal, you rise and bid the creature farewell, for now.");
		...
		//room 7 mess hall
			...
			Printer.println("You Descend the long set of stairs into the mess hall where some conflict seems to have ensued. Benches and tables are flipped over, arrows sticking out of nearly every surface, telling red stains litter the area. Old food rots upon the ground, discarded, unremembered. There are two doors, one to the south that leads up to the West Barracks and another that leads east to the Main Hall.");
			Enemy awaken = new Enemy();
			int pop = (int)(Math.random()*2+1);
			createEnemy(awaken, pop);
			combat(hero, awaken);
			...
			while (room7 == true && hero.getHP() > 0) {
				...
				Printer.println("4) Look at Map");
				Printer.println("5) Check resourcces");
				if (hero.getPotions() > 0) Printer.println("6) Drink a potion");
				Printer.print("> ");
				try {
					choice = input.nextInt();
		...
		//room 8 great hall
			...
			Printer.println("You enter the great hall. The ceiling is far above here, a good two stories high and curved. Faded art adorns the ceiling, which many pillars along the side of the room hold up. Great stained glass windows allow the red light from outside to filter in as many shades. Reds, blues, and greens splash across the floor, illuminating a large symbol in the middle with a covered lump upon it. There is only one door out, back towards the mess hall, all the others boarded up and blockaded.");
			Enemy awaken = new Enemy();
			int pop = 3;
			createEnemy(awaken, pop);
			combat(hero, awaken);
		...
		//room 10 shop (edit for coins)
			...
			Printer.println("As you enter the mysterious room, you find it lit by torches set upon the walls. A large, ornate carpet covers the ground upon which there is a wooden desk. Sitting at the desk is a hooded figure, the only visible feature a hooked nose poking out from beneath the hood along with long, stringy black hair. The figure motions with a bony hand at a chair that has suddenly appeared before the desk. You take a seat and the hooded figure rasps, \"I have some wares for you. Interested?\"");
							...
							if (choice == 1) {
								...
								while (potion == true) {
									...
									try {
										...
										Printer.println("Invalid input, exiting potion buying.");
										input.nextLine();
										amount = 0;
									} 
									if (hero.getCoins() >= (amount*2) && amount > 0) {
										...
										Printer.println("You do not have enough coins to buy this many potions.");
									}
									else if (amount == 0) {
		...
		//room 11 minor hallway
			...
			while (room11 == true && hero.getHP() > 0) {
				...
				if (choice == 1) {
					if (hero.isCaptainChamberKey()) {
						Printer.println("You enter the Captain's Chambers.");
						room = 12;
						room11 = false;
					}
				...
				if (choice == 1) {
					if (hero.getDMG() != 25) {
						...
						while (puzzle == true) {
							...
							if (choice == 1) {
								Printer.println("Enter a code for the six letter safe code (ex: Turkey)");
								Printer.print("> ");
								String guess = "";
								try {
									guess = input.next();
								...
								if (guess.equalsIgnoreCase(answer) == true) {
									Printer.println("You hear a click and swing the safe open to reveal a shimmering light. You reach in and grab it at which point your own weapon begins to glow with the same light, transforming.");
									hero.setDMG(25);
									hero.setShield(2);
									hero.setMana(20);
									puzzle = false;
					...
					else Printer.println("You find nothing new in this room, the safe already opened and the contents taken.");
				}
				else if (choice == 2) {
	...
	public static void combat(Player hero, Enemy foe) throws InterruptedException {
		...
		if (type == 1) {
			generateEnemy(foe);
			Printer.println("A soft wail fills the air as a wisp stumbles from the shadows towards you. " + foe.toString() + " It pauses to study you a moment before shuffling forward, hands outstretched.");
			while (hero.getHP() > 0 && foe.getHP() > 0) {
				...
				if (foe.getHP() > 0) {
					...
					if (huh == 1) Printer.print("The wisp swipes at your chest");
					else if (huh == 2) Printer.print("The wisp grasps at your throat");
					else Printer.print("The wisp tries to grab you in a hug");
					int chance = (int)(Math.random()*20+1);
					if (chance == 1) Printer.println(" but misses entirely!");
					else if (chance == 20) {
						foe.setDMG(foe.getDMG() + 2);
						int take = hero.takeDMG(foe.getDMG());
						foe.heal(foe.getDMG());
						Printer.println(" dealing " + take  + " damage and you notice it hit a little harder that time and seems to have healed itself with what it took from you.");
						
						
					}
					else {
						int take = hero.takeDMG(foe.getDMG());	
						Printer.println(" dealing " + take + " damage and you feel some of your life force slipping away!");
					}
				}
			}
			Printer.println("You successfully slay the wisp which fades away to nothing, leaving behind two shiny coins.");
			hero.setCoins(hero.getCoins()+2);
		...
		//husk fight
			...
			while (hero.getHP() > 0 && foe.getHP() > 0) {
				...
				if (foe.getHP() > 0) {
					...
					if (choice == 1) {
						if (foe.getHP() == 25 || foe.getShield() > 1) {
							int chance = (int)(Math.random()*20+1);
							if (chance == 1) Printer.println("The husk swipes at you with a fist but misses entirely.");
							else if (chance == 20) {
								Printer.println("The husk lunges with surprising speed and slams right into a weak spot in your defense.");
						...
						Printer.println("The husk swipes its leg around in a kick, connecting with your knee.");
						hero.takeDMG(foe.getDMG());
					}
				}
			}
			Printer.println("You successfully slay the husk which crumbles and turns to dust. You find four coins on the body.");
		...
		//mutant fight
			...
			generateEnemy(foe);
			Printer.println("There is a crackling in the air as you enter the room before out of nowhere a creature steps, its many red eyes glowering at you." + foe.toString() + " The mutant crackles with magical energy as it strikes the air with a whip made of acid, preparing for a fight.");
			while (hero.getHP() > 0 && foe.getHP() > 0) {
				//player turn
				choice = processPlayerTurn(hero, foe);
				//mutant turn
			...
			Printer.println("You successfully slay the mutant which howls and explodes into a burst of mana energy, leaving behind a small pouch of coins.");
			hero.setCoins(hero.getCoins()+10);
			if (hero.getDefendCharge() > 0) {
				hero.setDefendCharge(0);
				hero.takeSheild(3);
	...
	public static int processPlayerTurn(Player hero, Enemy foe) throws InterruptedException {
		...
		while (turn) {
			choice = hero.turn();
			if (choice == 1) {
				int taken = foe.takeDMG(hero.getDMG());
				int holder = hero.getDMG();
				if (holder == 4 || holder == 9) Printer.println("You bring around your staff, bonking your opponent for" + taken + " damage!");
				else if (holder == 5 || holder == 10) Printer.println("You jab quickly with your dagger, doing " + taken + " damage to the enemy!");
				else if (holder == 8 || holder == 13) Printer.println("You slash with your swords blade, cutting the foe for " + taken + " damage!");
				else if (holder == 12 || holder == 17) Printer.println("With a mighty swing your axe connects, doing " + taken + " damage!");
				else if (holder == 25 || holder == 30) Printer.println("You thrust the sunblade forward with a surety, seering your opponent for " + taken + " damage!");
				turn = false;
			}
			else if (choice == 2) {
				Printer.println("You take a defensive stance.");
				if (hero.getShield() == 0) hero.addShield(3);
				hero.addDefendCharge(3);
				turn = false;
			}
			else if (choice == 3) {
				boolean magic = true;
				while (magic) {
					try {
						...
						Printer.println("2) See Weakness (2 mana)");
						...
						if (holdMag == 1) {
							...
							if (hero.getDMG() == 25 || hero.getDMG() == 4 || hero.getDMG() == 8 || hero.getDMG() == 12 || hero.getDMG() == 5) {
								if (hero.getMana() >= 4) {
									Printer.println("You grasp your weapon and a surge of light passes down your arms into it, making it more deadly.");
							...
							if (hero.getDMG() == 3 || hero.getDMG() == 25 || hero.getDMG() == 9 || hero.getDMG() == 30) {
								if (hero.getMana() >= 3) {
									Printer.println("You brandish your weapon which glows with light before a burst erupts and slams into your foe, searing them as you feel your own vigor boosted.");
									hero.takeMana(3);
									foe.setHP(foe.getHP()-10);
									hero.heal(2);
									magic = false;
									turn = false;
								}
	...
	/**
	 * A very simple method that randomly selects premade descriptors for enemy objects to create variety.
	 * <pre>Example:
	...
	/**
	 * This method turns blank slate Enemy objects and turns them into a premade type of enemy, so as to allow randomization in other methods that call this one to implement that randomness properly.
	 * <pre>Examples:
	 * {@code createEnemy(foe, 1) will turn the foe Enemy object into the 1 type of enemy (wisps), editing its data to reflect this in combat as well assigning it that type value for later reference.
	 * }</pre>
	 * @param foe (object; the enemy object that holds the enemy data)
...
/**
 * <h1>Printer</h1>
 * <p>This allowed me to print lines with delays (so the text sprawled out instead of jumping onto the screen) as well as print arrays without making a loop each time.</p>
 * <p>Created: 1/11/2021</p> 
 * @author Jason Snow
 *
 */
class Printer {
	 ...
	 * }</pre>
	 * @param myArray (String[][]; the array to be printed)
	 */
	 ...
	 * }</pre>
	 * @param hold (String; the String to print)
	 * @throws InterruptedException (allows the method to function)
	 */
...
class Enemy extends Character {
	private int type;
	/**
	 * This method is a no argument constructor for the enemy type which makes an enemy type the player will not fight (mainly so if it did show up, it would be obvious something was wrong)
	 * <pre>Example:
	...
	/**
	 * This is the getter method for the enemy type.
	 * <pre>Example:
	...
	/**
	 * <pre>Example:
	 * {@code toString() might return a string like "The small wisp is shadowy, wearing a tattered shirt and has short pure white hair."
	 * }</pre>
...
class Player extends Character {
	...
	/**
	 * Getter method for a player objects potions.
	 * <pre>Example:
	...
	/**
	 * Setter method for potions for a player object
	 * <pre>Example:
	...
	/**
	 * Getter method for the coins data
	 * <pre>Example:
	...
	/**
	 * Getter method for the room data
	 * <pre>Example:
	...
	/**
	 * Setter method for the room data
	 * <pre>Example:
	...
	/**
	 * The getter method for the boolean westernBarracksKey data
	 * <pre>Example:
	...
	/**
	 * The getter method for the captainChamberKey data
	 * <pre>Example:
	...
	/**
	 * The getter method for the squid data which represents choices in the squid encounter.
	 * <pre>Example:
	...
	public String statString() {
		String hold = "HP: " + getHP() + " Mana: " + getMana() + " Damage: " + getDMG() + " Shield: " + getShield()  + " Coins: " + coins;
...
class Character{
	...
	/**
	 * Getter method for the shield data
	 * <pre>Example:
	...
	/**
	 * Getter method for defendCharge data
	 * <pre>Example:
	...
	/**
	 * Getter method for the character object height
	 * <pre>Example:
	...
	/**
	 * Getter method for the character object's color
	* <pre>Example:
	...
	/**
	 * Getter method for the character object's hairColor
	 * <pre>Example:
	...
	/**
	 * Setter method for the character object's hairLength
	 ** <pre>Example:
	...
	/**
	 * This method edits the HP value by subtracting from it to represent suffering damage. It automatically factors in the current shield and reduces the damage by that much. Later edited to return the actual damage taken so as to present the information to the user.
	 * <pre>Example:
	 ...
	 * @return take (int; the amount of damage actually taken)
	 */
	public int takeDMG(int taken) {
	...
	/**
	 * The getter method for the clothes of a character object
	 * <pre>Example:
